...
int main(){
    ...
    if (read_I2C(file,0x32,the_data) < 0) {
        close(file);
        exit(1);
    }

    initala[0] = (int)the_data[1]*256+the_data[0];
    initala[1] = (int)the_data[3]*256+the_data[2];
    initala[2] = (int)the_data[5]*256+the_data[4];

    while(read_I2C(file,0x32,the_data) <! 0){
    
    printf("=================================================\n");
    printf("X軸為%f\n",((int)the_data[1]*256+(int)the_data[0] - initala[0])/(float)256);
    printf("Y軸為%f\n",((int)the_data[3]*256+(int)the_data[2] - initala[1])/(float)256);
    printf("Z軸為%f\n",((int)the_data[5]*256+(int)the_data[4] - initala[2])/(float)256);
